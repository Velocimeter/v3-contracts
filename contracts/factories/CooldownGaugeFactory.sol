...
contract CooldownGaugeFactory is IGaugeFactory, Ownable {
    ...
    function createGauge(address _pool, address _external_bribe, address _ve, bool isPair, address[] memory allowedRewards) external returns (address) {
        last_gauge = address(new CooldownGauge(_pool, _external_bribe, _ve, msg.sender, oFlow, address(this), isPair, allowedRewards));
        if (oFlow != address(0)) {
    ...
    function updateOFlowFor(address _gauge) external onlyOwner {
        CooldownGauge(_gauge).setOFlow(oFlow);
