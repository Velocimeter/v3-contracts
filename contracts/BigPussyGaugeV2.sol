...
contract GaugeV2 is IGauge {
    ...
    constructor(address _stake, address _external_bribe, address  __ve, address _voter, address _oFlow, address _gaugeFactory, bool _forPair, address _router, address[] memory _allowedRewardTokens) {
        stake = _stake;
    ...
    */
    ...
    function buyAndDeposit(uint paymentAmount, uint _minOut, uint _deadline) public lock { 
        _deposit(msg.sender, paymentAmount, _minOut, _deadline);
    }

    function _deposit(address account, uint amount, uint _minOut, uint _deadline) private {
        require(amount > 0);
        _updateRewardForAllTokens();

        _safeTransferFrom(wFTM, msg.sender, address(this), amount);

        uint256 flowBefore = balanceOfFlow();
        IRouter(router).swapExactTokensForTokensSimple(amount, _minOut, wFTM, flow, false, address(this), _deadline);
        uint256 flowAfter = balanceOfFlow();
        uint256 flowResult = flowAfter - flowBefore;
        totalSupply += flowResult;
        balanceOf[account] += flowResult;
        ...
        emit Deposit(account, flowResult);
    }
