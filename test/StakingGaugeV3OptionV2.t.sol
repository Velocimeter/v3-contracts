...
contract StakingGaugeV3OptionV2 is BaseTest {
    ...
    function testClaimOFlowReward2Owner1() public {
        ...
        uint256 before = oFlowV2.balanceOf(address(owner));

        address[] memory rewards = new address[](1);
        rewards[0] = address(FLOW);
        gauge.getReward(address(owner), rewards);
        uint256 after_ = oFlowV2.balanceOf(address(owner));
