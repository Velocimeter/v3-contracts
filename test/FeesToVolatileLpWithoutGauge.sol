...
contract FeesToVolatileLpWithoutGauge is BaseTest {
    ...
    //     FRAX.approve(address(router), TOKEN_100K);
    //     WETH.approve(address(router), TOKEN_100K);
    //     (,, uint256 liquidity) = router.addLiquidityETH{value: TOKEN_100K}(address(FRAX), false, TOKEN_100K, TOKEN_100K, TOKEN_100K, address(owner2), block.timestamp);
    //     vm.stopPrank();

    //     Router.route[] memory routes = new Router.route[](1);
    //     routes[0] = Router.route(address(FRAX), address(WETH), false);

    //     uint256[] memory expectedOutput = router.getAmountsOut(TOKEN_100, routes);
    //     FRAX.approve(address(router), TOKEN_100);
    //     uint256[] memory amounts = router.swapExactTokensForETH(TOKEN_100, expectedOutput[1], routes, address(owner), block.timestamp);
    ...
    //     router.swapExactETHForTokens{value: TOKEN_100}(expectedOutput[1], routes, address(owner), block.timestamp);
    ...
    //     router.removeLiquidityETH(address(FRAX), false, liquidity, amountFRAX, amountETH, address(owner2), block.timestamp);
    //     vm.stopPrank();
